...
# Audio constants from your old server
MAX_MIC_GAIN = 5.0
VOLUME_STEPS = 10
PARROT_MIN_PCT = 8
PARROT_MAX_PCT = 20
...
print(f"🎵 Looking for WAVs in: {AUDIO_DIR.absolute()}")
print(f"🎵 WAVs found: {[f.name for f in AUDIO_DIR.glob('*.wav')]}")
...
# ===== YOUR EXACT ROUTES FROM OLD SERVER =====
@app.route("/")
def index():
    ...
    try:
        ...
        for i, dev in enumerate(devices):
            if "hifiberry" in dev['name'].lower() or "pcm5102a" in dev['name'].lower():
                print(f"✅ Found HifiBerry: device {i} - {dev['name']}")
                return i
        return 0
    except:
        return 0
...
def audio_callback(outdata, frames, time_info, status):
    try:
        ...
        # Use mic_gain + parrot_pct from your old server
...
def play_wav(filename):
    ...
    if not filepath.exists():
        print(f"❌ WAV missing: {filepath}")
        return
    try:
        print(f"🎵 Playing: {filename}")
        data, fs = sf.read(str(filepath))
        
        # Parrot speed effect using parrot_pct
        parrot_factor = 1.0 + (parrot_pct / 100.0)
